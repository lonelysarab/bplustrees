...
class TurtleFrame extends Frame {
	...
	void drawFractal(Graphics g, int n, String grammarString, Turtle t) {
        int i = 0;
        Turtle copy = null;
		if (grammarString.length() == 0) return;
        while (i < grammarString.length()) {
                        if (grammarString.charAt(i) == 'F')
                                if (n > 1)
                                        drawFractal(g,n-1, grammar.FText.getText(), t);
                                else 
                                        t.Forward(this,g);
                        else if (grammarString.charAt(i) == 'L')
                                if (n > 1) 
                                        drawFractal(g,n-1, grammar.LText.getText(), t);
                                else  
                                       t.Forward(this,g);
                        else if (grammarString.charAt(i) == 'R')
                                if (n > 1) 
                                        drawFractal(g,n-1, grammar.RText.getText(), t);
                                else 
                                      t.Forward(this,g);
                        else if (grammarString.charAt(i) == 'f')
                                if (n > 1) 
                                        drawFractal(g,n-1, grammar.fText.getText(), t);
                                else 
                                        t.Skip();
                        else if (grammarString.charAt(i) == '+')
                                t.Left(angle);
                        else if (grammarString.charAt(i) == '-')
                                t.Right(angle);
                        else if (grammarString.charAt(i) == '[') {

...
public class Stack extends LinearList {
	...
	if (linespernode==1) 
	    Leny=Textheight*1.45;
	else
	    Leny=(linespernode*Textheight)+
		((linespernode-1)*(0.45*Textheight));
	...
	//	System.out.println("Titleheight = " + Titleheight + " Lenx = " + Lenx + " Leny = " + Leny);
	...
	// Draw 3d top of stack and queue. *)
        ...
        drawArrow(Startx+(0.33333333*Lenx),Starty+(0.5*Textheight)+
		  (ArrowPercentofLenx*Lenx),Down,llist,d);
        drawArrow(Startx+(0.66666667*Lenx),Starty+(0.5*Textheight),
		  Up,llist,d);
	nodelist.reset();
